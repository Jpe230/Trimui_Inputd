...
// Definitions from stock daemon
#define SUNXI_PIO_PHYS_BASE     0x0300B000u
#define SUNXI_PIO_LM_PHYS_BASE  0x07022000u

// Per-bank stride in the TrimUI build: 0x24 bytes
#define SUNXI_GPIO_BANK_STRIDE  0x24u

// Offsets within each bank
#define OFF_CFG0   0x00u  // cfg[0]
#define OFF_DAT    0x10u  // dat
#define OFF_PULL0  0x1Cu  // pull[0]

// pin encoding: bank = pin >> 5, num = pin & 31
#define GPIO_BANK(pin) ((uint32_t)((pin) >> 5))
#define GPIO_NUM(pin)  ((uint32_t)((pin) & 0x1Fu))

#define GPIO_CFG_INDEX(pin)  (((pin) & 0x1Fu) >> 3)          // 0..3
...
int sunxi_gpio_set_cfgpin(uint32_t pin, uint32_t val) {
    ...
    uint32_t idx = GPIO_CFG_INDEX(pin);   // 0..3
    uint32_t off = GPIO_CFG_OFFSET(pin);  // 0..28 step 4
